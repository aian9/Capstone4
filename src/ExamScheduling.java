...
public class ExamScheduling
{
    ...
    public static boolean checkRandomTimeslot(int randomCourse, int 
            randomTimeslot, int[][] conflictMatrix, int[] schedule)
    {
        for(int i=0; i<conflictMatrix.length; i++)
            if(conflictMatrix[randomCourse][i] !=0 && schedule[i]==randomTimeslot)
                return false;
        return true;              
    }
